#!/usr/bin/env bash
# zabbix_export_pgsql.sh - Minimal PostgreSQL bundle exporter

set -euo pipefail

readonly SCRIPT_VERSION="1.0"

# Keep exclusions aligned with importer flow.
...
# FK-safe load order for importer.
...
# Emit MySQL-compatible CSV payload to keep one importer path.
export_table_pgsql() {
    local table="$1"
    local out_file="$2"
    ...
    [[ ${#col_names[@]} -eq 0 ]] && { log "No columns for ${table}, skipping"; return 1; }
...
main() {
    ...
    log "Zabbix PostgreSQL export script v${SCRIPT_VERSION}"
