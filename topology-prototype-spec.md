...
Recents
Use cases for tag-based topology model
7 minutes ago
Теги и проблема рассинхронности на продакшене
23 hours ago
Защита подхода тэгов для хранения топологии
24 hours ago
Топология сети через LLD и host prototypes
2 days ago
Zabbix topology roadmap planning
2 days ago
Гибридная модель с таблицей и тэгами
2 days ago
Перерисовка картинки
5 days ago
Переход на реляционную базу для тегов
5 days ago
Инструкции агенту для модели с тэгами
5 days ago
Distinguishing hosted_by relationships in topology spec
5 days ago
Перемещение связи represented_by в topo_nodes
6 days ago
Pull operation and topo_nodes host registration
6 days ago
Data-driven event architecture
6 days ago
Хранение данных топологии на тегах
6 days ago
Хранение разных типов связей в одной таблице
6 days ago
Переосмысление MEMBER_OF_LAG как структурной принадлежности
6 days ago
Instructions
Каждый ответ перед выдачей покритикуй внутри себя и давай вывод уже на основе этой критики
...
topology-tags-desync-tradeoffs.md
3.2kB
...
topology-tags-vs-model-debate.md
5.8kB
...
- Edge type: `physical_link` (Port ↔ Port, the only relationship stored in
  `topo_edges`). `represented_by` (Device → Host/Proxy) is **not** an edge —
  it's three columns directly on the `Device`'s `topo_nodes` row
  (`represented_by_node_id`/`represented_by_matched_by`/`represented_by_at`,
  §2.1) — and LAG membership is likewise a structural `Port` reference —
  `Port.lag_id` — not an edge type; see §2.1/§2.3 for why in both cases.
...
- Neighbor `Device`↔`Host` matching: MAC-only, opportunistic (§3 rule 2).
  ...
  gets a `represented_by` representation set.
...
CREATE TABLE topo_nodes (
  ...
  -- set only when type='device'; the Device's current representation as a Host or Proxy.
  -- Reconsidered from a topo_edges edge type into a node-level field — see the note after
  -- this table and §2.3 for why represented_by does not need generic edge infrastructure.
  represented_by_node_id    BIGINT NULL UNIQUE REFERENCES topo_nodes(id) ON DELETE SET NULL,
  represented_by_matched_by VARCHAR(16) NULL,   -- 'mac' | 'reporter_self' | 'manual'; set only alongside represented_by_node_id
  represented_by_at         TIMESTAMP NULL,     -- when the CURRENT representation was established; set only alongside represented_by_node_id
...
**`represented_by` is a field on `Device`, not a `topo_edges` row — reconsidered
after the rest of this section was originally written.** Unlike
`physical_link`, `represented_by` is directed, has at most one *active*
representation per `Device` (not single-valued across the `Device`'s whole
history — a `Device` can be promoted, depromoted, and promoted again to a
different target over time), is never confirmed independently by multiple
reporters, and has no stale/reconciliation lifecycle of its own. It doesn't
need generic edge infrastructure, so it's stored directly on the `Device`
row instead, the same `topo_nodes.id` namespace `Host`/`Proxy` already live
in (a native self-referencing FK, not the polymorphic external reference
rejected in §12 — `ON DELETE SET NULL` behaves the same way `device_id`'s
and `lag_id`'s FKs already do).
 
**Two invariants, enforced at the application layer, not the database** —
same category as the `lag_id IS NOT NULL ⇒ target.if_type = 'lag'` rule
above:
 
1. `represented_by_node_id IS NOT NULL ⇒` the target node's `type ∈ {host,
   proxy}`.
2. `represented_by_node_id IS NULL ⇒ represented_by_matched_by IS NULL` and
   `represented_by_at IS NULL` — the two provenance fields exist only
   alongside an active representation. `/depromote` (§6) must clear all
   three fields together, never just `represented_by_node_id`.
`matched_mac` (the specific MAC that produced a `mac`-matched representation)
is deliberately **not** persisted anywhere in this shape: nothing in the API,
UI, or §8's acceptance criteria consumes it, and it would represent a
potentially stale snapshot (`host.inventory.macaddress_a/b` can change after
the match) rather than live topology state. A reconciliation audit/debug
history, if one is ever needed, is a separate, more general mechanism (a
decision log, not a `topo_nodes` column) — filed under §11's
"discovery-quality visibility" item, not built into this table on the
strength of hypothetical future use.
 
**Net effect on this section and §2.3: `topo_edges` now holds only
`physical_link`.** This isn't just the removal of one edge type — it leaves
`topo_edges` semantically homogeneous, every remaining row an independently
evolving relationship with its own multi-source lifecycle (§2.3's
`last_seen_src`/`last_seen_dst` reasoning applies to all of it, with no
edge-type exception to carve out anymore). Read every mention of a
"`represented_by` edge" elsewhere in this document (§2.2, §2.3, §3, §4.1, §5,
§6, §7, §8, §11) as shorthand for "a non-null `represented_by_node_id`,
with `represented_by_matched_by` recording how it was established" —
the underlying rules (1:1 cardinality both directions, manual-override
independence, never silently replaced, etc.) are unchanged from when they
were written against an edge; only the storage shape moved.
...
CREATE TABLE topo_edges (
  id         BIGINT PRIMARY KEY AUTO_INCREMENT,
  type       VARCHAR(32) NOT NULL,   -- 'physical_link' (the only edge type — represented_by moved to topo_nodes, see above)
...
would be blocked by any `physical_link` edge still pointing at a `Port` on
the associated `Device` — its own `Port`s aren't touched by the `Host`
deletion directly, but the `Device`'s `represented_by_node_id` pointing at
the now-deleted `Host` row does need `ON DELETE SET NULL` (already on that
column, §2.1 above) so the `Device` cleanly reverts to unassociated rather
than the deletion being blocked or the pointer left dangling. This was
found via a real scenario, not hypothetically: deleting a Zabbix `Host`
that already had topology data attached needs the full chain to work
cleanly, not just the first hop.
 
**Generic storage does not imply generic constraints — say this explicitly
so a future reader doesn't assume otherwise.** `topo_edges` now holds only
`physical_link` (`represented_by` moved to `topo_nodes` — see above), but
even a single-type table still needs three separate pieces of type-specific
database logic layered on top of its generic shape: pair uniqueness, source
uniqueness, destination uniqueness (all detailed in §2.3). A shared table
does not mean a new edge type, if one is ever added, is free — it brings
its own constraint logic that has to be designed, not inherited
automatically from the generic schema.
 
**Edge-level uniqueness needs care for the one edge type that remains —
`physical_link` duplicate/reversed-pair prevention (detailed in §2.3),
enforced on `(src_id, dst_id)` scoped to `type='physical_link'`.** Since
`topo_edges` is now homogeneous (one type only), this no longer needs the
partial-index/generated-column engine-specific workaround that a
type-scoped uniqueness check on a mixed table would have required — a
plain composite index/constraint on `(src_id, dst_id)` is enough, together
with the separate per-port uniqueness on `src_id` and `dst_id` individually
described in §2.3. `represented_by`'s 1:1-both-directions requirement is no
longer an edge-uniqueness question at all: it's the plain `UNIQUE`
constraint already on `topo_nodes.represented_by_node_id` (§2.1's DDL
above), which — because it's a single column holding at most one target per
`Device` row — enforces both directions for free: one `Device` can't point
at two targets (it's one column), and `UNIQUE` stops two `Device`s from
...
`represented_by` (Device → Host or Proxy) is **not** in this table —
reconsidered into three columns directly on the `Device`'s `topo_nodes` row
(`represented_by_node_id`, `represented_by_matched_by`,
`represented_by_at`; see §2.1). It's still the same relationship
conceptually (a `Device`'s current representation as a `Host`/`Proxy`,
established `"mac"`/`"reporter_self"`/`"manual"`), just no longer stored as
a generic edge — it doesn't need one, per §2.1's reasoning. `matched_mac`
is not persisted (§2.1). Every rule below written against a
"`represented_by` edge" — 1:1 cardinality, manual-override independence,
the split-topology/stacked-switch limitations — still holds; only the
...
trivially true since `represented_by_node_id` is a single column), and a
given `Host`/`Proxy` can be the target of at most one `Device`'s
representation. Enforce the second half with the plain `UNIQUE` constraint
already on `topo_nodes.represented_by_node_id` (§2.1) — no partial index or
edge-type scoping needed, since the column only ever exists on `Device`
rows. Creating a second representation for either side must fail, or must
first require an explicit `/depromote` (§6) — never silently overwrite the
existing one.
...
same `Device` — the 1:1 rule above blocks the second `/promote` (a `Device`
row has only one `represented_by_node_id` column; nothing stops it pointing
at a *different* `Host`, which is exactly why a second `/promote` must be
rejected rather than silently overwriting the first). This is a
conscious MVP simplification, not an oversight: supporting it would require
a structurally different shape — a `Device` able to hold more than one
representation, which this single-column design deliberately doesn't allow
— and would break §7's rendering design, which assumes exactly one
...
`Host`s, `topo_nodes.represented_by_node_id`'s `UNIQUE` constraint means one
`Host` can't be `represented_by` by two `Device`s. This blocks representing
**stacked switches** (multiple physical chassis, each with its own
`chassis_id` over LLDP, managed and monitored as one logical Zabbix host)
or an **MLAG pair** — both are common enterprise topologies, more likely to
be hit than the split-identity case above. Same root cause, same fix if
it's ever needed (a structurally different shape allowing more than one
`Device` to point at the same `Host`/`Proxy`, plus a §7 rendering redesign
to show more than one Device section on a single `Host`/`Proxy` panel),
same "pick one `Device` to represent it, leave the rest topology-only or
manually linked" workaround for now.
...
`/promote` is the explicit fallback for every case it doesn't cover. A
representation set manually stays manual — there's no promotion to
`"lldp"`-equivalent provenance over time the way `physical_link` has (§3.5)
— it stays `matched_by: "manual"` unless someone `/depromote`s it and a
fresh match happens some other way.
...
3. **`Device` nodes are never deleted** when its `represented_by`
   representation is cleared (e.g. a Zabbix host is decommissioned). The
   physical device may still exist and participate in the network; only
   `represented_by_node_id`/`represented_by_matched_by`/`represented_by_at`
   (§2.1) revert to `NULL`, the `Device` row itself stays.
...
correct (possibly pre-existing) `Device` is resolved this way, its
`represented_by_node_id`/`represented_by_matched_by`/`represented_by_at`
columns (§2.1) are set to that exact `Host`, no opportunistic matching
involved, subject to the same `represented_by` 1:1 constraint as any other
case (§2.1, §2.3) — if that `Device` somehow already has a *different*
active representation, don't silently override it, log and skip, same as
any other conflict. Set `represented_by_matched_by='reporter_self'` to
distinguish this from an opportunistic MAC match.
...
- For each `Host` returned, attempt Device reconciliation per §3.2 against
  ...
  `attrs`, see §2.2), then `UPDATE` the matched `Device`'s row, setting
  `represented_by_node_id` to that `Host` node's id and
  `represented_by_matched_by='mac'`/`represented_by_at=now()` together
  (§2.1) — not a `topo_edges` insert. No match → no node, no representation
  set, nothing written for that host. (Monitoring
...
- `POST /topo/devices/{id}/promote {host_id}` — manually set this
  `Device`'s representation, where `host_id` is a **Zabbix `hostid`** (from
  the search endpoint above), not a `topo_nodes` id — the target `Host` may
  not have a `topo_nodes` row yet. This endpoint must itself upsert the
  `Host` node (`host_ref=hostid`) if it doesn't already exist, using the
  same upsert-not-insert discipline as §3 rule 4, then `UPDATE` the
  `Device`'s own row, setting `represented_by_node_id` to that `Host`
  node's id, `represented_by_matched_by='manual'`, and
  `represented_by_at=now()` together (§2.1's second invariant — the three
  columns are always set or cleared as a unit) — not a `topo_edges` insert.
  A second `/promote` against an already-materialized `Host` must not
  create a duplicate node. Deliberate user action, never automatic, and
  does **not** run the §3.2 strong-key check — see the "manual override"
  and "1:1 cardinality" notes under §2.3 for the exact rules this endpoint
  must enforce (reject if either side already has an active
  representation — the `Device`'s own `represented_by_node_id` is already
  non-null, or the target `Host`/`Proxy`'s `topo_nodes.id` is already
  referenced as someone else's `represented_by_node_id`, which the column's
  `UNIQUE` constraint will itself reject at the database level as a
  backstop). A `{proxy_id}` variant (a Zabbix `proxyid`) works the same way
  for `Proxy`.
- `POST /topo/devices/{id}/depromote` — manually clear this `Device`'s
  representation: `UPDATE` its `topo_nodes` row, setting
  `represented_by_node_id`, `represented_by_matched_by`, and
  `represented_by_at` all to `NULL` together (§2.1's second invariant) — not
  a `topo_edges` delete. No soft-delete or `valid_to` marker (temporal
  versioning is out of scope, see §1). Does not touch any other column on
  the `Device` node — it reverts to the same unassociated state described
  in §3.3. **Also deletes the `Host`/`Proxy` `topo_nodes` row this `Device`
  was pointing at, unless something else still gives it a reason to exist**
...
- **`represented_by` 1:1 constraint — the most likely redesign candidate if
  ...
  specific cases and what loosening the constraint would require (since
  §2.1's refactor, this is no longer just an index change — a single
  `represented_by_node_id` column on `Device` can only ever hold one
  target, so allowing multiple representations means either a real
  multi-valued structure there or reintroducing a join table for this one
  relationship, plus a §7 rendering redesign to show more than one Device
  section on a single `Host`/`Proxy` panel, per that discussion).
...
- **Manual logical relationships beyond `hosted_by`** (`depends_on`,
  `communicates_with`, `replicates_to`, and similar) — a broader version of
  the same category `hosted_by` above already opens: relationships that
  have real topology meaning but that LLDP can never see and that hold
  independent of physical adjacency. Raised as a candidate during design
  review, alongside the explicit question of whether to let a person draw
  an arbitrary manual link between two `Host`s (or a `Host` and a `Device`,
  or a `Host` and a future `Service`) directly in the UI. **Decided: no —
  not as a single generic "connect any two nodes" edge.** A plain manual
  Host↔Host line has no fixed meaning (physical? dependency? cluster
  membership? "these belong together" for documentation?), and a graph
  where edges don't reliably mean one thing stops being trustworthy to
  read. If/when a relationship in this category is actually built, it
  must follow the same discipline `hosted_by` and `physical_link` already
  follow, not a shortcut: **its own `topo_edges` type value, its own
  declared set of valid src/dst node types, and its own cardinality rule**
  — decided at the point that specific type is designed, same as every
  other edge type in §2.1/§2.3, never inherited for free from the fact
  that `topo_edges` already exists. This is deliberately **not** a
  polymorphic "any node → any node with a label" mechanism: that shape was
  already rejected once, for a different relationship, in §12
  (`object_ref` instead of separate `host_ref`/`proxy_ref` — rejected for
  losing referential integrity and the cascade guarantee a typed FK gives
  for free). The same reasoning applies here: a generic labeled-connector
  edge type would reintroduce that exact loose polymorphism one layer up,
  just between edges instead of between node references. Concretely, this
  means a UI that lets a person pick "Depends on" / "Hosted by" /
  "Replicates to" from a fixed list backed by real, individually-designed
  edge types is in scope for a future iteration; a UI that lets a person
  draw an unlabeled or freely-labeled line between two arbitrary nodes is
  not, and should not be built even as a stopgap. No specific new edge
  type is committed to here beyond `hosted_by` (already its own item
  above) — this entry exists to record the "typed edge per relationship,
  never a generic connector" rule itself, so it governs whichever of
  `depends_on`/`communicates_with`/`replicates_to`/etc. gets designed
  first, rather than that decision being re-litigated from scratch.
...
- **Tags are flat key-value strings, not typed relationships with
  structured attributes.** `represented_by` carries
  `matched_by`/`represented_by_at` (§2.1); `physical_link` carries
  `discovered_via`/`last_seen` (§2.3). Encoding this in tags means either
  one tag per attribute (multiplying per node or edge) or serializing a
  blob into a tag value — both break the normal tag-based filtering/search
  tags exist for in the first place.
