...
#define ZBX_CONFIG_CSV_RC_USAGE		1
#define ZBX_CONFIG_CSV_RC_NOT_IMPLEMENTED	2
#define ZBX_CONFIG_CSV_RC_IO		3
#define ZBX_CONFIG_CSV_RC_VALIDATION	4
...
typedef struct
{
	zbx_config_csv_buf_t	buf;
	const char		*pos;
	const char		*end;
}	zbx_config_csv_reader_t;

typedef int (*zbx_csv_write_callback_t)(const void *arg, const char *data, size_t len);
...
static int zbx_config_csv_buf_append_len(zbx_config_csv_buf_t *buf, const char *data, size_t len)
{
	if (SUCCEED != zbx_config_csv_buf_grow(buf, buf->len + len + 1))
		return FAIL;
...
static void zbx_config_csv_free_row(char **fields, int *is_null, size_t field_count)
{
	for (size_t i = 0; i < field_count; i++)
	{
		if (NULL != fields[i])
...
typedef struct
{
	...
	const char	*output;
}	zbx_config_csv_opts_t;

static void print_usage(void)
{
	printf("Zabbix Configuration Migration Utility\n");
	printf("Usage: zabbix_config_csv <command> [options]\n\n");
	printf("Commands:\n");
	printf("  export   Export Zabbix database to CSV bundle\n");
	printf("  import   Import CSV bundle to Zabbix database\n");
	printf("  validate Validate CSV bundle integrity\n");
	printf("  upgrade  Upgrade Zabbix database schema\n\n");
	printf("Options:\n");
	printf("  --db-type mysql|postgresql   Database type (required for export/import/upgrade)\n");
	printf("  --db-host HOST               Database host (default: localhost)\n");
	printf("  --db-port PORT               Database port\n");
	printf("  --db-name NAME               Database name\n");
	printf("  --db-user USER               Database user\n");
	printf("  --db-password PASSWORD       Database password\n");
	printf("  --output BUNDLE.tar.gz       Output bundle path (for export)\n");
	printf("  --input BUNDLE.tar.gz        Input bundle path (for import/validate)\n");
}
...
int main(int argc, char **argv)
{
	...
	{
		print_usage();
		return ZBX_CONFIG_CSV_RC_USAGE;
	}

	const char *command = argv[1];

	if (0 == strcmp(command, "-h") || 0 == strcmp(command, "--help"))
	{
		print_usage();
		return SUCCEED;
	}
	...
	{
		fprintf(stderr, "zabbix_config_csv export: not yet implemented\n");
		return ZBX_CONFIG_CSV_RC_NOT_IMPLEMENTED;
	}
	else if (0 == strcmp(command, "import"))
	{
		fprintf(stderr, "zabbix_config_csv import: not yet implemented\n");
		return ZBX_CONFIG_CSV_RC_NOT_IMPLEMENTED;
	}
	else if (0 == strcmp(command, "validate"))
	{
		fprintf(stderr, "zabbix_config_csv validate: not yet implemented\n");
		return ZBX_CONFIG_CSV_RC_NOT_IMPLEMENTED;
	}
	else if (0 == strcmp(command, "upgrade"))
	{
		fprintf(stderr, "zabbix_config_csv upgrade: not yet implemented\n");
		return ZBX_CONFIG_CSV_RC_NOT_IMPLEMENTED;
	}
	else
	{
