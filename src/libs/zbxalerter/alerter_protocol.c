...
zbx_uint32_t	zbx_alerter_serialize_email(unsigned char **data, zbx_uint64_t alertid, zbx_uint64_t mediatypeid,
		const char *name, int maxattempts, zbx_uint64_t eventid, zbx_uint64_t p_eventid, zbx_uint64_t actionid,
		int source, int object, zbx_uint64_t objectid, int message_type, const char *sendto,
		...
		const char *recovery_expression)
{
	...
	zbx_serialize_prepare_value(data_len, p_eventid);
	zbx_serialize_prepare_value(data_len, actionid);
	...
	*data = (unsigned char *)zbx_malloc(NULL, data_len);
	...
	ptr += zbx_serialize_value(ptr, p_eventid);
	ptr += zbx_serialize_value(ptr, actionid);
...
void	zbx_alerter_deserialize_email(const unsigned char *data, zbx_uint64_t *alertid, zbx_uint64_t *mediatypeid,
		char **name, int *maxattempts, zbx_uint64_t *eventid, zbx_uint64_t *p_eventid, zbx_uint64_t *actionid,
		int *source, int *object, zbx_uint64_t *objectid, int *message_type, char **sendto, char **subject,
		...
		char **expression, char **recovery_expression)
{
	...
	data += zbx_deserialize_value(data, p_eventid);
	data += zbx_deserialize_value(data, actionid);
...
static void	alerter_serialize_mediatype(unsigned char **data, zbx_uint32_t *data_alloc, zbx_uint32_t *data_offset,
		...
		const char *script, const char *timeout, zbx_uint64_t oauthprofileid)
{
	...
	zbx_serialize_prepare_value(data_len, oauthprofileid);
	...
	(void)zbx_serialize_value(ptr, oauthprofileid);
		...
		unsigned char *message_format, char **script, char **timeout, zbx_uint64_t *oauthprofileid)
{
	...
	data += zbx_deserialize_value(data, oauthprofileid);
		...
		const char *message, const char *params)
{
	...
	*data = zbx_malloc(0, data_alloc);
	alerter_serialize_mediatype(data, &data_alloc, &data_offset, mediatypeid, type, name, smtp_server, smtp_helo,
			...
			message_format, script, timeout, oauthprofileid);
...
{
	unsigned char	*ptr;
	zbx_uint32_t	data_len = 0, params_len, oauth_bearer_len;
	...
	zbx_serialize_prepare_str_len(data_len, params, params_len);
	zbx_serialize_prepare_str_len(data_len, oauth_bearer, oauth_bearer_len);
	...
	*data = (unsigned char *)zbx_malloc(NULL, data_len);
	...
	ptr += zbx_serialize_str(ptr, oauth_bearer, oauth_bearer_len);
		...
		char **params, char **oauth_bearer, unsigned char *debug)
{
	...
	data += zbx_deserialize_str(data, oauth_bearer, len);
		...
		int mediatypes_num)
{
	...
	for (int i = 0; i < mediatypes_num; i++)
	{
		...
		alerter_serialize_mediatype(data, &data_alloc, &data_offset, mt->mediatypeid, mt->type, mt->name,
				...
				mt->attempt_interval, mt->message_format, mt->script, mt->timeout, mt->oauthprofileid);
	}
...
zbx_uint32_t	zbx_alerter_serialize_alerts(unsigned char **data, zbx_am_db_alert_t **alerts, int alerts_num)
{
	...
	for (int i = 0; i < alerts_num; i++)
	{
		...
		zbx_serialize_prepare_value(data_len, alert->actionid);
		zbx_serialize_prepare_value(data_len, alert->min_alertid);
		zbx_serialize_prepare_value(data_len, alert->message_type);
		...
		ptr += zbx_serialize_value(ptr, alert->actionid);
		ptr += zbx_serialize_value(ptr, alert->min_alertid);
		ptr += zbx_serialize_value(ptr, alert->message_type);
...
void	zbx_alerter_deserialize_alerts(const unsigned char *data, zbx_am_db_alert_t ***alerts, int *alerts_num)
{
	...
	*alerts = (zbx_am_db_alert_t **)zbx_malloc(NULL, *alerts_num * sizeof(zbx_am_db_alert_t *));
	...
	for (int i = 0; i < *alerts_num; i++)
	{
		...
		data += zbx_deserialize_value(data, &alert->actionid);
		data += zbx_deserialize_value(data, &alert->min_alertid);
		data += zbx_deserialize_value(data, &alert->message_type);
...
zbx_uint32_t	zbx_alerter_serialize_send_dispatch(unsigned char **data, const zbx_db_mediatype *mt,
		const zbx_vector_str_t *recipients, zbx_uint64_t eventid, int message_type)
{
	unsigned char	*ptr;
...
void	zbx_alerter_deserialize_send_dispatch(const unsigned char *data, zbx_db_mediatype *mt,
		zbx_vector_str_t *recipients, zbx_uint64_t *eventid, int *message_type)
{
	zbx_uint32_t	len;
	int		recipients_num;
		...
		const zbx_vector_str_t *recipients, zbx_uint64_t eventid, int message_type, char **error)
{
	...
	size = zbx_alerter_serialize_send_dispatch(&data, mediatype, recipients, eventid, message_type);
