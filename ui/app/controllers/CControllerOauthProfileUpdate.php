...
class CControllerOauthProfileUpdate extends CController {
	...
	public static function getValidationRules(): array {
		return ['object', 'fields' => [
			'oauthprofileid' => ['db oauth_profile.oauthprofileid', 'required'],
			...
			'client_secret' => ['db oauth_profile.client_secret'],
	...
	protected function checkInput(): bool {
		...
		if (!$ret) {
			...
			$response = $form_errors
				...
				: ['error' => [
					'title' => _('Cannot update OAuth profile'),
	...
	protected function doAction(): void {
		$oauthprofileid = $this->getInput('oauthprofileid');

		$db_profile = DBfetch(DBselect(
			'SELECT oauthprofileid'.
			' FROM oauth_profile'.
			' WHERE oauthprofileid='.zbx_dbstr($oauthprofileid)
		));
		...
		if ($this->hasInput('client_secret') && $this->getInput('client_secret') !== '') {
			$profile['client_secret'] = $this->getInput('client_secret');
		}

		if ($this->hasInput('access_token') && $this->getInput('access_token') !== '') {
			$profile += [
				'tokens_status' => (int) $this->getInput('tokens_status', 0),
				'access_token' => $this->getInput('access_token'),
				'access_token_updated' => (int) $this->getInput('access_token_updated', 0),
				'access_expires_in' => (int) $this->getInput('access_expires_in', 0),
				'refresh_token' => $this->getInput('refresh_token', '')
			];
		}

		DB::update('oauth_profile', [[
			'values' => $profile,
			'where' => ['oauthprofileid' => $oauthprofileid]
		]]);

		$this->setResponse(new CControllerResponseData([
			'main_block' => json_encode([
				'success' => [
					'title' => _('OAuth profile updated')
				]
			])
		]));
	}
